    ...
    private String createS3Path(FileInfoDto fileInfoDto) {
        ...
        switch (fileInfoDto.getS3Option()) {
            case profileUpload -> {     //프로필 사진 업로드 경로
                return "ProfileImage/" + fileInfoDto.getUserId() + "/" + UUID.randomUUID().toString() + ext;    //프로필 이미지 저장경로 생성
            }
            ...
            case galleryUpload -> {     //갤러리 사진 업로드 경로
                if(fileInfoDto.getDate() != null) {
                    ...
                    return "Gallery/" + fileInfoDto.getUserId() + "/" + dateString + "/" + UUID.randomUUID().toString() + originName;
                }
                return null;
            }
            case galleryDelete -> {
